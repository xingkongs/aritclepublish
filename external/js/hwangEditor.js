...
_e(function (E, $) {
    E.createMenu(function (check) {
        ...
        */
        ...
        menu.updateSelectedEvent = function () {
            var rangeElem = editor.getRangeElem();
            rangeElem = editor.getSelfOrParentByName(rangeElem, 'h2');
            if (rangeElem) {
                return true;
            }
            return false;
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        var $div2 = $div1.clone();
        var $div3 = $div1.clone().css('margin', '0 10px').css('text-align','center');
        var $textInput = $('<input type="text" class="block" placeholder="显示的文字"/>');
        var $urlInput = $('<input type="text" class="block" placeholder="http://"/>');
        var $btnSubmit = $('<button class="right--re">' + lang.submit + '</button>');
        var $btnCancel = $('<button class="right--re gray">' + lang.cancel + '</button>');

        $div1.append($textInput);
        $div2.append($urlInput);
        $div3.append($btnCancel).append($btnSubmit);
        $content.append($div2).append($div1).append($div3);
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        // 创建 panel 内容
        var $content = $('<div></div>');
        var $linkInputContainer = $('<div style="margin:25px 10px;"></div>');
        var $linkInput = $('<input type="text" class="block" placeholder="http://"/>');
        $linkInputContainer.append($linkInput);
        var $sizeContainer = $('<div style="margin:20px 10px;display:none;"></div>');
        ...
        var $headContent=$('<p style="margin-top:10px;margin-left:10px;color:#3b3b3b;font-size: 16px;">插入视频</p>')
        var $howToCopy = $('<p style="margin-top:10px;margin-left:10px;margin-bottom:18px;color:#cacaca;font-size: 14px;">目前支持优酷、土豆、腾讯等视频url</p>');
        var $btnSubmit = $('<button class="right--re">' + lang.submit + '</button>');
        var $btnCancel = $('<button class="right--re gray">' + lang.cancel + '</button>');
        $btnContainer.append($btnCancel).append($btnSubmit);
        $content.append($headContent).append($linkInputContainer).append($sizeContainer).append($howToCopy).append($btnContainer);
        ...
        $btnSubmit.click(function (e) {
            ...
            // }
            if (!reg.test(link)){
                var _url = $('input.block').val();
                if (!(/https?\:\/\/\S+(youku|tudou|qq|iqiyi|sohu)\.([a-z\.]+)\/\S+/i.test(_url)) && _url.length < 8){
                    alertContent("暂不支持该视频服务,请输入分享链接")
                }
                $.ajax({
                    type: "GET",
                    url: 'http://www.d1ev.com/api/video/player',
                    dataType: 'json',
                    data: {url: encodeURI(_url)},
                    success: function (msg) {
                        //console.log(msg);
                        //console.log(msg.errmsg);
                        if (typeof (msg.errmsg) != 'undefined') {
                            alertContent(msg.errmsg);
                            return false;
                        }
                        var _iframe = '<p><iframe class="video-iframe" frameborder="0" width="640" height="498" src="' + msg.embed + '" allowfullscreen></iframe></p>';
                        _iframe = _iframe + '<p><a class="video-link" href="' + msg.src + '" data-src="' + msg.src + '" data-title="' + msg.title + '" data-embed="' + msg.embed + '"></a></p>'
                        // 执行命令
                        editor.command(e, 'insertHtml', _iframe);
                        $linkInput.val('');
                        //console.log(_iframe)
                        //menu.dropPanel.focusFirstInput();
                        return;
